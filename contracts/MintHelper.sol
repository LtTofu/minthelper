...
contract MintHelper is Ownable {
  ...
  using SafeMath for uint;
    ...
    // called from the mining software.  identify the rewards that will be won and how to split them up
    function proxyMint( uint256 nonce, bytes32 challenge_digest, address mintableToken )
    public /* onlyOwner */
    returns (bool)
    {
    
      address minersAddress = address( uint160(nonce) );                    //destination (user)'s address from the first 20 bytes specified by the first 20 bytes of the nonce [TESTME]
      ...
      //transfer the tokens to the correct wallets.
      require(ERC20Interface(mintableToken).transfer(minterWallet, minterReward));
      require(ERC20Interface(mintableToken).transfer(minersAddress, payoutReward));
