...
//identify the rewards that will be won and how to split them up:
    ...
    public /*onlyOwner*/
    returns (bool)
    { // trying to get miner's payout address from first 20 bytes of the solution nonce...
    
        address payoutAddress = address( uint160(nonce) );            // <--  get most-significant 160 bits, truncate the rest -or-...
    //  address payoutAddress = address(uint160( uint256(nonce) ));   // <--- least-significant 160 bits instead? [WIP].
      
        uint totalReward = ERC918Interface(mintableToken).getMiningReward();  // get current reward from the ERC918 contract
        uint minterReward = totalReward.mul(2).div(100);                      // developer gets 2%  
    ...
    //transfer the tokens to the correct wallets.
        ...
        require(ERC20Interface(mintableToken).transfer(payoutAddress, payoutReward));
